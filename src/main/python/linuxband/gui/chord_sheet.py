...
class ChordSheet(object):
  ...
  def drawing_area_realize_event_callback(self, widget):
    ...
    #self.pixmap = Gdk.Pixmap(self.drawable, self.__drawing_area_width, self.__drawing_area_height, depth= -1)
  ...
  def drawing_area_draw_event_callback(self, widget, cr):
  ...
  def drawing_area_motion_notify_event_callback(self, widget, event):
    # selection by mouse
    if event.get_state() & BUTTON1_MASK:
      pos = self.__locate_mouse_click(event.x, event.y)
      old_pos = self.__cursor_pos
      if not pos == old_pos:
        self.__move_cursor_to(pos)
  ...
  def __init_gui(self, glade):
    ...
    self.__drawing_area_width = self.__area.get_allocated_width()
    self.__drawing_area_height = self.__area.get_allocated_height()
    self.__bar_width = self.__drawing_area_width / self.__song.get_data().get_beats_per_bar()
    self.__bar_chords_width = self.__bar_width * 9 / 10
    self.__bar_info_width = self.__bar_width - self.__bar_chords_width
    self.__area.show()
  ...
  def __render_chord_xy(self, chord, x, y, width, height, playhead):
    ...
    if db is not None:
      ...
      cc.translate(x, y + (height - ink[1] - ink[3]))
      cc.show_page()
      db.flush()
    else:
      print('Invalid double buffer')
  ...
  def __render_chords_xy(self, bar_num, chords, bar_x, bar_y, playhead, cursor, selection):
    ...
    if db is not None:
      cc = cairo.Context(db)
      cc.set_source_rgb(color.red, color.green, color.blue)
      cc.rectangle(bar_x , bar_y, self.__bar_chords_width, ChordSheet.__bar_height)
      cc.fill()
      if cursor: # black border
        color = Gdk.color_parse("black")
        cc.set_source_rgb(color.red, color.green, color.blue)
        cc.rectangle(bar_x , bar_y, self.__bar_chords_width - 1, ChordSheet.__bar_height - 1)
        cc.stroke()
      if not chords:
        return
      if playhead:
  ...
  def __draw_repetition(self, x, y, cc, end):
    # draw line
  ...
  def __render_bar_info_xy(self, bar_num, chord_num, bar_info, x, y, cursor, selection):
      ...
      if db is not None:
        cc = cairo.Context(db)
        cc.set_source_rgb(color.red, color.green, color.blue)
        cc.rectangle(x, y, self.__bar_info_width - 1, ChordSheet.__bar_height - 1)
        cc.fill()
        if cursor:
          color = Gdk.color_parse("black")
          cc.set_source_rgb(color.red, color.blue, color.blue)
          cc.rectangle(x, y, self.__bar_info_width - 1, ChordSheet.__bar_height - 1)
          cc.stroke()
        ...
        if repeat_begin or repeat_end: # draw repetitions
          if repeat_begin: self.__draw_repetition(x, y, cc, False)
          if repeat_end: self.__draw_repetition(x, y, cc, True)
        else:
          if bar_num < self.__song.get_data().get_bar_count() and chord_num: # draw bar number
            ...
            pc.update_layout(pango_layout)
            pc.show_layout(pango_layout)
            cc.translate(x + ChordSheet.__cell_padding, y + (ChordSheet.__bar_height - ink[1] - ink[3]) - ChordSheet.__cell_padding)
            cc.show_page()
            db.flush()
      else:
        print("Invalid double buffer")
