...
class Gui:
    ...
    def switch_bar(self, on_bar_chords):
        """
        If we are not on chords field hide the chord entries and show events. Called by ChordSheet.
        """
        if on_bar_chords:
    ...
    def refresh_bar(self, on_bar_chords):
        """ Refresh chord_entries or events_bar. Called by ChordSheet"""
        if on_bar_chords:
    ...
    def main_window_keypress_event_callback(self, widget, event):
        """ Signal handler for key_press_event in GtkWindow 'mainWindow' """
        key = event.keyval
    ...
    def edit_cut_callback(self, menuitem):
        """ Signal handler for activate in GtkAction 'imagemenuitem6' stock-id=gtk-cut """
    ...
    def edit_copy_callback(self, menuitem):
        """ Signal handler for activate in GtkAction 'imagemenuitem7' stock-id=gtk-copy """
        if self.__chord_sheet.has_focus():
    ...
    def edit_paste_callback(self, menuitem):
        """ Signal handler for activate in GtkAction 'imagemenuitem8' stock-id=gtk-paste """
        if self.__chord_sheet.has_focus():
    ...
    def edit_delete_callback(self, menuitem):
        """ Signal handler for activate in GtkAction 'imagemenuitem9' stock-id=gtk-delete """
        if self.__chord_sheet.has_focus():
    ...
    def edit_select_all_callback(self, menuitem):
        """ Signal handler for activate in GtkAction 'imagemenuitem12' stock-id=gtk-select-all """
        if self.__chord_sheet.has_focus():
    ...
    def view_preferences_callback(self, menuitem):
        """ Preferences. Signal handler for activate in GtkAction 'imagemenuitem9' stock-id=gtk-preferences """
    ...
    def spinbutton_keyrelease_event_callback(self, widget, event):
        """ For songBarCount and introLength spin buttons . Signal handler for key_release_event in GtkSpinButton 'spinbutton3' """
        key = event.keyval
        if key == Gdk.KEY_Return or key == Gdk.KEY_KP_Enter or key == Gdk.KEY_Escape:
            self.__chord_sheet.grab_focus()
        return True

    def song_bar_count_value_changed_callback(self, widget):
        """ Signal handler for value_changed in GtkSpinButton 'spinbutton1' """
        self.change_song_bar_count(widget.get_value_as_int())

    def intro_length_value_changed_callback(self, widget):
        """ Signal handler for value_changed in GtkSpinButton 'spinbutton3' """
    ...
    def chord_sheet_button_press_event_callback(self, widget, event):
        """ Signal handler for button_press_event in GtkDrawingArea 'drawingarea1' """
    ...
    def song_title_keyrelease_event_callback(self, widget, event):
        """ Finish Song title editing. Signal handler for key_release_event GtkEntry 'entry9' """
        key = event.keyval
    ...
    def jack_reconnect_callback(self, button):
        """Signal handler for clicked in GtkToolButton 'toolbutton13' """
        self.__midi_player.shutdown()
        self.__midi_player.startup();

    def switch_page_callback(self, notebook, page, pageNum):
        """ Called when clicked on notebook tab. Signal handler for switch_page in GtkNotebook 'notebook3' """
        logging.debug("")
    ...
    def loop_toggle_callback(self, button):
        """ Loop check button. Signal handler for toggled in GtkCheckButton 'checkbutton1' """
    ...
    def jack_transport_toggle_callback(self, button):
        """ Use JACK transport check button. Signal handler for toggled in GtkCheckButton 'checkbutton3' """
    ...
    def __init_recent_menu(self, glade):
        ...
        recent_menu_chooser = Gtk.RecentChooserMenu.new_for_manager(manager)
        ...
        recent_menu_chooser.set_sort_type(Gtk.RecentSortType.MRU)
        ...
        #menuitem6 = glade.get_object("menuitem6")
        menuitem6 = Gtk.MenuItem("Recent songs")
        menuitem6.set_submenu(recent_menu_chooser)
        # attach the RecentChooserMenu to the Open tool button
        toolbutton8 = glade.get_object("toolbutton8")
        toolbutton8.set_menu(recent_menu_chooser)

    def __init_filechooser_dialogs(self, glade):
        self.__open_file_dialog = glade.get_object("openFileDialog")
        self.__save_as_dialog = glade.get_object("saveAsDialog")
        self.__export_midi_dialog = glade.get_object("exportMidiDialog")
    ...
    def __init__(self):
        glade = Gtk.Builder()
        glade.add_from_file(Glob.GLADE)
        GuiLogger.initLogging(glade)
        Common.connect_signals(glade, self)

        self.__main_window = glade.get_object("mainWindow")
        self.__spinbutton1 = glade.get_object("spinbutton1") # bar count 
        self.__notebook2 = glade.get_object("notebook2")
        self.__notebook3 = glade.get_object("notebook3")

        # song name
        self.__entry9 = glade.get_object("entry9")
        self.__entry9.modify_font(Pango.FontDescription('10'))

        # global buttons
        self.__global_buttons = glade.get_object("vbox10")

        # pause button
        self.__toolbutton3 = glade.get_object("toolbutton3")

        # view menu toggle
        self.__menuitem5 = glade.get_object("menuitem5")
        self.__menuitem7 = glade.get_object("menuitem7")

        # hack to get event object when clicked on toolbutton
        toolbutton1 = glade.get_object("toolbutton1")
        toolbutton1.get_children()[0].connect('button-press-event', self.playback_start)

        # save changes dialog
        self.__save_changes_dialog = glade.get_object("saveChangesDialog")
